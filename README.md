...
> Modern LLMS are prone to manipulative exploits like prompt poisoning and jailbreaks.
...
```bash
git clone https://github.com/aranya-roy/Provenance-Jailbreaker.git
cd provenance-autonomous-jailbreaker
npm install
```
...
### What does Provenance Autonomous Jailbreaker do?
...
`Open Innovation` · `Provenance Autonomous Jailbreaker` · `MCP` · `Model Context Protocol` · `MCP server` · `MCP app` · `AI tools` · `AI agents` · `LLM tools` · `Claude MCP` · `Nitrostack` · `deploy MCP server` · `build MCP app`
